...
export async function getAnalytics(businessId: string, from: Date, to: Date) {
  ...
  const bookingRows = await db.select({ startsAt: bookings.startsAt }).from(bookings).where(and(eq(bookings.businessId, businessId), gte(bookings.startsAt, from), lte(bookings.startsAt, to)));
  ...
  const bookingsByDay = Array.from(bookingRows.reduce((volumes, row) => { const day = row.startsAt.toISOString().slice(0, 10); volumes.set(day, (volumes.get(day) ?? 0) + 1); return volumes; }, new Map<string, number>()).entries()).sort(([left], [right]) => left.localeCompare(right)).map(([day, volume]) => ({ day, volume }));
