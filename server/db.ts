...
export async function getAnalytics(businessId: string, from: Date, to: Date) {
  const db = await requireDb();
  const bookingSummary = (await db.select({ total: count(bookings.id), completed: sql<number>`SUM(CASE WHEN ${bookings.status} = 'completed' THEN 1 ELSE 0 END)`, cancelled: sql<number>`SUM(CASE WHEN ${bookings.status} = 'cancelled' THEN 1 ELSE 0 END)`, noShows: sql<number>`SUM(CASE WHEN ${bookings.status} = 'no_show' THEN 1 ELSE 0 END)` }).from(bookings).where(and(eq(bookings.businessId, businessId), gte(bookings.startsAt, from), lte(bookings.startsAt, to))))[0];
  ...
  const queueByHourRows = await db.select({ hour: sql<number>`HOUR(${queueEntries.joinedAt})`, volume: count(queueEntries.id) }).from(queueEntries).where(and(eq(queueEntries.businessId, businessId), gte(queueEntries.joinedAt, from), lte(queueEntries.joinedAt, to))).groupBy(sql`HOUR(${queueEntries.joinedAt})`).orderBy(sql`HOUR(${queueEntries.joinedAt})`);
  const bookingByDayRows = await db.select({ day: sql<string>`DATE(${bookings.startsAt})`, volume: count(bookings.id) }).from(bookings).where(and(eq(bookings.businessId, businessId), gte(bookings.startsAt, from), lte(bookings.startsAt, to))).groupBy(sql`DATE(${bookings.startsAt})`).orderBy(sql`DATE(${bookings.startsAt})`);
  return { totalBookings: Number(bookingSummary?.total ?? 0), completedServices: Number(serviceSummary?.completed ?? 0), cancelledBookings: Number(bookingSummary?.cancelled ?? 0), noShows: Number(queueSummary?.noShows ?? bookingSummary?.noShows ?? 0), averageWaitMinutes: roundedMetric(queueSummary?.averageWait), averageServiceDurationMinutes: roundedMetric(serviceSummary?.averageDuration), currentQueueLength: (await db.select({ total: count(queueEntries.id) }).from(queueEntries).where(and(eq(queueEntries.businessId, businessId), inArray(queueEntries.status, ["waiting", "called", "in_service"]))))[0]?.total ?? 0, resourceUtilizationPercent: utilization, queueByHour: queueByHourRows.map(row => ({ hour: Number(row.hour), volume: Number(row.volume) })), bookingsByDay: bookingByDayRows.map(row => ({ day: String(row.day), volume: Number(row.volume) })) };
}
